...
void ayiya_reader(char *buf, unsigned int length)
{
	...
	lenout = (int)send(ayiya_socket->socket, (const char *)&s2, length, 0);
#else
...
#endif
{
	...
	while (true)
	{
		cl = sizeof(ci);
		memset(buf, 0, sizeof(buf));
		n = (int)recvfrom(ayiya_socket->socket, (char *)buf, sizeof(buf), 0, (struct sockaddr *)&ci, &cl);
...
#if defined(_FREEBSD) || defined(_DFBSD) || defined(_OPENBSD) || defined(_DARWIN) || defined(_NETBSD)
	lenout = (int)send(ayiya_socket->socket, (const char *)&s, (unsigned int)n, 0);
#else
