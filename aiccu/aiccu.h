...
#ifdef AICCU_PATCH
enum TKZAiccuError {
     TKZAiccuErrInvalidUser = 2,
     TKZAiccuErrInvalidPwd = 3,
     TKZAiccuErrNoTicConnection = 4,
     TKZAiccuErrClientNotUpToDate = 5,
     TKZAiccuErrNoTimeSync = 6,
     TKZAiccuErrLoadConfig = 7,
     TKZAiccuErrNoTunnel = 8
    };
#endif
...
struct AICCU_conf
{
	...
	int		tunrunning;		/* Is the tundev running? */
    
	int		automatic;		/* Try to be totally automatic? */
	int		behindnat;		/* Behind a NAT */
	int		requiretls;		/* Require TLS for TIC? */
	int		makebeats;		/* Make heartbeats? */
	int		noconfigure;		/* No configuration (used to only send heartbeats) */
	int		defaultroute;		/* Configure a default route */
#endif
};
